...
function FraudControlPage({token}){
  ...
  const [saving,setSaving]=useState(false);
  const [result,setResult]=useState(null);
  const load=()=>{Promise.all([apiFetch("/fraud/overview",token),apiFetch("/fraud/rules",token),apiFetch("/fraud/events",token),apiFetch("/fraud/blocks",token)]).then(([ov,r,e,b])=>{setOverview(ov);setRules(r.data||[]);setEvents(e.data||[]);setBlocks(b.data||[]);});};
  useEffect(()=>{load();const t=setInterval(load,15000);return()=>clearInterval(t);},[token]);
  ...
  return(<div style={{minHeight:"100vh",background:"#F2F2F2",fontFamily:"Arial,Helvetica,sans-serif"}}>
    <div style={{background:"#FFF",borderBottom:"1px solid #E0E0E0",padding:"12px 16px"}}><div style={{fontSize:18,fontWeight:700}}>🛡 Fraud Control</div><div style={{fontSize:11,color:"#999",marginTop:2}}>Monitoring mode</div></div>
    <div style={{padding:"12px 16px"}}>
      {result&&<div style={{padding:"10px 14px",borderRadius:8,marginBottom:12,background:result.success?"rgba(16,185,129,0.1)":"rgba(239,68,68,0.1)",border:"1px solid "+(result.success?"#10B981":"#EF4444"),fontSize:12,color:result.success?"#10B981":"#EF4444",fontWeight:600}}>{result.success?"✅ ":"❌ "}{result.message||result.error}</div>}
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12}}>
        {[{l:"Open",v:overview.open_events||0,c:"#EF4444",i:"🚨"},{l:"Blocks",v:overview.active_blocks||0,c:"#F97316",i:"🚫"},{l:"Today",v:overview.today_events||0,c:"#F59E0B",i:"📅"},{l:"CPS",v:overview.cps_current||0,c:"#2CADA6",i:"⚡"},{l:"Calls",v:overview.concurrent_calls||0,c:"#3B82F6",i:"📞"},{l:"Critical",v:overview.critical_events||0,c:"#DC2626",i:"🔴"}].map((c,i)=>(
          <div key={i} style={{background:"#FFF",borderRadius:8,padding:"10px 12px",boxShadow:"0 1px 4px rgba(0,0,0,0.06)",textAlign:"center"}}><div style={{fontSize:10,marginBottom:2}}>{c.i}</div><div style={{fontSize:20,fontWeight:800,color:c.c}}>{c.v}</div><div style={{fontSize:9,color:"#999",fontWeight:600,textTransform:"uppercase"}}>{c.l}</div></div>
        ))}
      </div>
      <div style={{display:"flex",gap:4,marginBottom:12,overflowX:"auto"}}>
        {[["overview","📊 Overview"],["rules","⚙ Rules"],["events","🚨 Events"],["blocks","🚫 Blocks"]].map(([t,l])=>(
      ...
      {tab==="rules"&&(<><div style={{display:"flex",justifyContent:"flex-end",marginBottom:10}}><button onClick={()=>setShowAddRule(!showAddRule)} style={{padding:"8px 16px",borderRadius:20,border:"none",background:"#2CADA6",color:"#FFF",fontSize:12,fontWeight:700,cursor:"pointer"}}>+ Add Rule</button></div>
        ...
        :<div style={{background:"#FFF",borderRadius:8,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr>{["NAME","TYPE","ACTION","THRESHOLD","STATUS",""].map((h,i)=><th key={i} style={thS}>{h}</th>)}</tr></thead><tbody>{rules.map((r,i)=>(<tr key={r.id} style={{borderBottom:"1px solid #F5F5F5",background:i%2===0?"#FFF":"#FAFAFA"}}><td style={{padding:"8px 10px",fontSize:12,fontWeight:600}}>{r.name}</td><td style={{padding:"8px 10px",fontSize:11}}>{r.type}</td><td style={{padding:"8px 10px",fontSize:11}}>{r.action}</td><td style={{padding:"8px 10px",fontSize:12,fontFamily:"monospace"}}>{r.threshold||"—"}</td><td style={{padding:"8px 10px"}}>{badge(r.status)}</td><td style={{padding:"8px 10px",textAlign:"center"}}><button onClick={async()=>{if(!window.confirm("Delete?"))return;await apiFetch("/fraud/rules/"+r.id,token,{method:"DELETE"});load();}} style={{background:"none",border:"1px solid #EF4444",borderRadius:4,cursor:"pointer",fontSize:11,color:"#EF4444",padding:"2px 6px"}}>Del</button></td></tr>))}</tbody></table></div>}
      </>)}
      {tab==="events"&&(events.length===0?<div style={{background:"#FFF",borderRadius:10,padding:40,textAlign:"center",color:"#999",fontSize:12}}>✅ No events</div>
        :<div style={{background:"#FFF",borderRadius:8,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr>{["TIME","TYPE","CLI","REASON","SEV","STATUS",""].map((h,i)=><th key={i} style={thS}>{h}</th>)}</tr></thead><tbody>{events.map((e,i)=>(<tr key={e.id} style={{borderBottom:"1px solid #F5F5F5"}}><td style={{padding:"6px 10px",fontSize:10,color:"#555"}}>{(e.created_at||"").slice(0,16)}</td><td style={{padding:"6px 10px",fontSize:11,fontWeight:600}}>{e.type}</td><td style={{padding:"6px 10px",fontSize:11,fontFamily:"monospace"}}>{e.src||"—"}</td><td style={{padding:"6px 10px",fontSize:11,color:"#555"}}>{e.reason}</td><td style={{padding:"6px 10px"}}><span style={{color:sev(e.severity),fontWeight:700,fontSize:11}}>{e.severity}</span></td><td style={{padding:"6px 10px"}}>{badge(e.status)}</td><td style={{padding:"6px 10px"}}>{e.status==="open"&&<button onClick={async()=>{await apiFetch("/fraud/events/"+e.id,token,{method:"PUT",body:JSON.stringify({status:"resolved"})});load();}} style={{background:"none",border:"1px solid #10B981",borderRadius:4,cursor:"pointer",fontSize:10,color:"#10B981",padding:"2px 6px"}}>Resolve</button>}</td></tr>))}</tbody></table></div>)}
      {tab==="blocks"&&(<><div style={{display:"flex",justifyContent:"flex-end",marginBottom:10}}><button onClick={()=>setShowAddBlock(!showAddBlock)} style={{padding:"8px 16px",borderRadius:20,border:"none",background:"#EF4444",color:"#FFF",fontSize:12,fontWeight:700,cursor:"pointer"}}>+ Block</button></div>
        ...
        :<div style={{background:"#FFF",borderRadius:8,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr>{["TYPE","VALUE","REASON","BY","STATUS",""].map((h,i)=><th key={i} style={thS}>{h}</th>)}</tr></thead><tbody>{blocks.map((b,i)=>(<tr key={b.id} style={{borderBottom:"1px solid #F5F5F5"}}><td style={{padding:"6px 10px"}}><span style={{fontSize:10,padding:"2px 6px",borderRadius:4,background:"#FEE2E2",color:"#EF4444",fontWeight:700}}>{(b.type||"").toUpperCase()}</span></td><td style={{padding:"6px 10px",fontSize:12,fontFamily:"monospace",fontWeight:600}}>{b.value}</td><td style={{padding:"6px 10px",fontSize:11,color:"#555"}}>{b.reason}</td><td style={{padding:"6px 10px",fontSize:11}}>{b.blocked_by}</td><td style={{padding:"6px 10px"}}>{badge(b.status)}</td><td style={{padding:"6px 10px"}}>{b.status==="active"&&<button onClick={async()=>{if(!window.confirm("Release?"))return;await apiFetch("/fraud/blocks/"+b.id+"/release",token,{method:"PUT"});load();}} style={{background:"none",border:"1px solid #10B981",borderRadius:4,cursor:"pointer",fontSize:10,color:"#10B981",padding:"2px 6px"}}>Release</button>}</td></tr>))}</tbody></table></div>}
      </>)}
    </div>
  </div>);
}
    ...
    <div style={{padding:"12px 16px"}}>
      ...
      :health&&(<>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:12}}>
          {health.checks&&["asterisk","nginx","php_fpm","mysql","api"].map(key=>{
            ...
            return(<div key={key} style={{background:"#FFF",borderRadius:8,padding:14,boxShadow:"0 1px 4px rgba(0,0,0,0.06)",borderLeft:"3px solid "+sC(c.status)}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}><div style={{fontSize:13,fontWeight:700}}>{sI(c.status)} {c.name}</div><span style={{fontSize:10,padding:"2px 8px",borderRadius:10,fontWeight:700,background:sC(c.status)+"20",color:sC(c.status)}}>{(c.status||"").toUpperCase()}</span></div>
              <div style={{fontSize:11,color:"#555"}}>{c.detail}</div>
              {c.latency&&<div style={{fontSize:10,color:"#999",marginTop:2}}>Latency: {c.latency}</div>}
        ...
        {health.checks?.sip&&(<>
          ...
          <div style={{background:"#FFF",borderRadius:8,padding:14,marginBottom:12,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
            {Object.entries(health.checks.sip.trunks||{}).map(([name,status])=>{const ok=status.toLowerCase().includes("avail")&&!status.toLowerCase().includes("unavail");return(<div key={name} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"1px solid #F5F5F5"}}><span style={{fontSize:12,fontWeight:600}}>{name}</span><span style={{fontSize:11,color:ok?"#10B981":"#F59E0B",fontWeight:600}}>{ok?"🟢":"🟡"} {status}</span></div>);})}
          </div>
        </>)}
        <div style={{fontSize:12,fontWeight:700,color:"#555",marginBottom:8,textTransform:"uppercase"}}>Resources</div>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {health.checks&&["cpu","memory","disk"].map(key=>{const c=health.checks[key];if(!c)return null;return(<div key={key} style={{background:"#FFF",borderRadius:8,padding:14,boxShadow:"0 1px 4px rgba(0,0,0,0.06)",borderLeft:"3px solid "+sC(c.status)}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:13,fontWeight:700}}>{sI(c.status)} {c.name}</span><span style={{fontSize:11,color:"#555"}}>{c.detail}</span></div>{c.percent&&<div style={{background:"#F0F0F0",borderRadius:4,height:6,overflow:"hidden"}}><div style={{width:c.percent+"%",height:"100%",background:sC(c.status),borderRadius:4}}/></div>}</div>);})}
        </div>
      </>)}
    </div>
  </div>);
...
function SIPMonitorPage({token}){
  ...
  const [ts,setTs]=useState("");
  const logRef=useRef(null);
  const load=useCallback(()=>{apiFetch("/sip/activity",token).then(d=>{setInvites(d.invites||[]);setTs(d.timestamp||"");const ac=(d.channels||[]).filter(c=>c.match(/(\d+) active call/)).map(c=>c.match(/(\d+) active call/)?.[1]).join("")||"0";setActiveCalls(ac);const epList=[];let cur=null;for(const line of (d.pjsip||[])){const m=line.match(/Endpoint:\s+([A-Z0-9_-]+)/);if(m){cur={name:m[1],status:"",rtt:"—"};epList.push(cur);}if(cur){const s=line.match(/(Avail|NonQual|Unavail|Not in use|In use)/);if(s&&!cur.status)cur.status=s[1];const r=line.match(/([\d.]+)$/);if(r&&cur.rtt==="—")cur.rtt=r[1]+"ms";}}setEps(epList);setLoading(false);});},[token]);
  ...
  return(<div style={{minHeight:"100vh",background:"#F2F2F2",fontFamily:"Arial,Helvetica,sans-serif"}}>
    ...
    <div style={{padding:"12px 16px"}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12}}>
        {[{l:"Active Calls",v:activeCalls,c:"#10B981",i:"📞"},{l:"Endpoints",v:eps.length,c:"#3B82F6",i:"🔌"},{l:"Events",v:invites.length,c:"#2CADA6",i:"📋"}].map((c,i)=>(
          <div key={i} style={{background:"#FFF",borderRadius:8,padding:12,boxShadow:"0 1px 4px rgba(0,0,0,0.06)",textAlign:"center"}}><div style={{fontSize:14,marginBottom:2}}>{c.i}</div><div style={{fontSize:22,fontWeight:800,color:c.c}}>{c.v}</div><div style={{fontSize:9,color:"#999",fontWeight:600,textTransform:"uppercase"}}>{c.l}</div></div>
        ))}
      </div>
      <div style={{display:"flex",gap:4,marginBottom:12}}>
        {[["invites","📋 INVITE History"],["endpoints","🔌 Endpoints"],["log","📄 Log"]].map(([t,l])=>(
      ...
      {tab==="invites"&&<div style={{display:"flex",flexDirection:"column",gap:8}}>
        ...
        <div style={{background:"#FFF",borderRadius:8,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
          ...
          :<div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:560}}>
            <thead><tr style={{background:"#2CADA6"}}>{["TIME","CALLER","DID","SUPPLIER","DUR","STATUS"].map((h,i)=><th key={i} style={{fontSize:9,color:"#FFF",fontWeight:700,padding:"6px 8px",textAlign:"left",textTransform:"uppercase",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
      ...
      {tab==="endpoints"&&<div style={{background:"#FFF",borderRadius:8,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr>{["ENDPOINT","STATUS","RTT"].map((h,i)=><th key={i} style={thS}>{h}</th>)}</tr></thead><tbody>{eps.map((ep,i)=>(<tr key={i} style={{borderBottom:"1px solid #F5F5F5",background:i%2===0?"#FFF":"#FAFAFA"}}><td style={{padding:"12px 10px",fontWeight:700,fontSize:13}}>{ep.name}</td><td style={{padding:"12px 10px"}}><span style={{fontSize:11,fontWeight:700,color:sColor(ep.status)}}>{sIcon(ep.status)} {ep.status||"Unknown"}</span></td><td style={{padding:"12px 10px",fontSize:11,fontFamily:"monospace"}}>{ep.rtt}</td></tr>))}</tbody></table></div>}
      {tab==="log"&&<div style={{background:"#1A1A2E",borderRadius:8,overflow:"hidden"}}><div style={{padding:"10px 14px",borderBottom:"1px solid rgba(255,255,255,0.1)",display:"flex",justifyContent:"space-between"}}><span style={{fontSize:11,fontWeight:700,color:"#FFF"}}>Asterisk Log</span><span style={{fontSize:10,color:"#555"}}>{log.length} lines</span></div><div ref={logRef} style={{maxHeight:400,overflowY:"auto",padding:"8px 0"}}>{log.map((line,i)=>(<div key={i} style={{padding:"2px 14px",fontFamily:"monospace",fontSize:9,lineHeight:1.6,wordBreak:"break-all",color:line.includes("ERROR")||line.includes("WARNING")?"#EF4444":line.includes("NOTICE")?"#F59E0B":"#6B7280"}}>{line}</div>))}</div></div>}
    </div>
  </div>);
}
...
function StatsPage({token}){
  ...
  const weeklyData=()=>{
    const weeks={};
    cdrs.forEach(c=>{
      const d=new Date(c.call_start||"");if(isNaN(d.getTime()))return;
      const wn=Math.ceil(d.getDate()/7);
      const key=d.getFullYear()+"-"+String(d.getMonth()+1).padStart(2,"0")+"-W"+wn;
      const label="W"+wn+"/"+String(d.getMonth()+1).padStart(2,"0");
  ...
  const SVGBar=({data,vk,color})=>{
    if(!data||!data.length)return(<div style={{padding:20,textAlign:"center",color:"#999",fontSize:12}}>No data for this period</div>);
    const max=Math.max(...data.map(d=>d[vk]||0),0.001);
    return(<div style={{overflowX:"auto"}}><svg width={Math.max(600,data.length*24)} height={190} style={{display:"block"}}>
      {data.map((d,i)=>{const h=Math.round((d[vk]||0)/max*140);const x=i*24+2;return(<g key={i}>
        <rect x={x} y={150-h} width={20} height={h||1} fill={color} rx={3} opacity={0.85}/>
        <text x={x+10} y={168} textAnchor="middle" fontSize={8} fill="#999">{d.date||""}</text>
        {h>14&&<text x={x+10} y={150-h-4} textAnchor="middle" fontSize={8} fill={color} fontWeight="bold">{parseFloat(d[vk]).toFixed(d[vk]<10?1:0)}</text>}
      </g>);})}
    </svg></div>);
  };

  return(<div style={{minHeight:"100vh",background:"#F2F2F2",fontFamily:"Arial,Helvetica,sans-serif"}}>
    <div style={{background:"#FFF",borderBottom:"1px solid #E0E0E0",padding:"12px 16px"}}><div style={{fontSize:18,fontWeight:700}}>📈 Statistics</div><div style={{fontSize:11,color:"#999",marginTop:2}}>Revenue and call analytics</div></div>
    <div style={{padding:"12px 16px"}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12}}>
        {[{l:"Total Calls",v:cdrs.length,c:"#3B82F6",i:"📞"},{l:"Minutes",v:Math.round(cdrs.reduce((a,c)=>a+parseInt(c.billsec||0),0)/60)+"m",c:"#2CADA6",i:"⏱"},{l:"Revenue",v:"€"+totalRevenue.toFixed(2),c:"#10B981",i:"💶"}].map((c,i)=>(
          <div key={i} style={{background:"#FFF",borderRadius:8,padding:12,boxShadow:"0 1px 4px rgba(0,0,0,0.06)",textAlign:"center"}}><div style={{fontSize:14,marginBottom:2}}>{c.i}</div><div style={{fontSize:18,fontWeight:800,color:c.c}}>{c.v}</div><div style={{fontSize:9,color:"#999",fontWeight:600,textTransform:"uppercase"}}>{c.l}</div></div>
        ))}
      </div>
      <div style={{display:"flex",gap:4,marginBottom:12,overflowX:"auto"}}>
      ...
      {loading?<div style={{padding:40,textAlign:"center",color:"#999"}}>Loading...</div>:(<>
        {tab==="daily"&&<div style={{display:"flex",flexDirection:"column",gap:12}}>
          <div style={{background:"#FFF",borderRadius:8,padding:12,boxShadow:"0 1px 4px rgba(0,0,0,0.06)",display:"flex",alignItems:"center",gap:10}}>
            ...
            <span style={{fontSize:11,color:"#999"}}>{dailyData().reduce((a,d)=>a+d.calls,0)} calls</span>
          </div>
          <div style={{background:"#FFF",borderRadius:8,padding:16,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><div style={{fontSize:12,fontWeight:700,marginBottom:10}}>Revenue €</div><SVGBar data={dailyData()} vk="revenue" color="#10B981"/></div>
          <div style={{background:"#FFF",borderRadius:8,padding:16,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><div style={{fontSize:12,fontWeight:700,marginBottom:10}}>Calls</div><SVGBar data={dailyData()} vk="calls" color="#3B82F6"/></div>
          <div style={{background:"#FFF",borderRadius:8,padding:16,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><div style={{fontSize:12,fontWeight:700,marginBottom:10}}>Minutes</div><SVGBar data={dailyData()} vk="minutes" color="#2CADA6"/></div>
        </div>}
        ...
        {tab==="table"&&<div style={{background:"#FFF",borderRadius:8,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr>{["DATE","DID","CLI","SUPPLIER","SEC","REVENUE","CUR"].map((h,i)=><th key={i} style={thS}>{h}</th>)}</tr></thead><tbody>{cdrs.slice(0,200).map((c,i)=>(<tr key={i} style={{borderBottom:"1px solid #F5F5F5",background:i%2===0?"#FFF":"#FAFAFA"}}><td style={{padding:"6px 10px",fontSize:10,color:"#555",whiteSpace:"nowrap"}}>{(c.call_start||"").slice(0,16)}</td><td style={{padding:"6px 10px",fontSize:11,fontFamily:"monospace",color:"#2CADA6",fontWeight:600}}>{c.did||"—"}</td><td style={{padding:"6px 10px",fontSize:11,fontFamily:"monospace"}}>{c.src||"—"}</td><td style={{padding:"6px 10px",fontSize:11,fontWeight:600}}>{c.trunk_name||"—"}</td><td style={{padding:"6px 10px",fontSize:11,fontFamily:"monospace"}}>{c.billsec||0}</td><td style={{padding:"6px 10px",fontSize:11,fontWeight:700,color:"#10B981",fontFamily:"monospace"}}>{c.currency==="USD"?"$":"€"}{parseFloat(c.revenue||0).toFixed(4)}</td><td style={{padding:"6px 10px",fontSize:10,color:"#555"}}>{c.currency||"EUR"}</td></tr>))}</tbody></table></div></div>}
      </>)}
    </div>
  </div>);
}
...
function TestNumbersPage({token}){
  ...
  const [selectedCountry,setSelectedCountry]=useState("");
  const [msg,setMsg]=useState(null);
  useEffect(()=>{Promise.all([apiFetch("/did-ranges",token),apiFetch("/dids",token),apiFetch("/suppliers",token)]).then(([r,d,s])=>{const trunks={};(s.data||[]).forEach(t=>{trunks[t.id]=t.nickname;});setRanges(r.data||[]);setDids((d.data||[]).map(x=>({...x,supplier_name:trunks[x.trunk_id]||"—"})));setLoading(false);});},[token]);
  const countries=[...new Set(ranges.map(r=>r.country_name).filter(Boolean))].sort();
  ...
  return(<div style={{minHeight:"100vh",background:"#F2F2F2",fontFamily:"Arial,Helvetica,sans-serif"}}>
    <div style={{background:"#FFF",borderBottom:"1px solid #E0E0E0",padding:"12px 16px"}}><div style={{fontSize:18,fontWeight:700}}>📋 Test Numbers</div><div style={{fontSize:11,color:"#999",marginTop:2}}>Select country to view test numbers</div></div>
    <div style={{padding:"12px 16px"}}>
      ...
      <div style={{background:"#FFF",borderRadius:10,padding:16,boxShadow:"0 1px 4px rgba(0,0,0,0.06)",marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <select value={selectedCountry} onChange={e=>setSelectedCountry(e.target.value)} style={{flex:1,padding:"10px 12px",border:"1px solid #E0E0E0",borderRadius:8,fontSize:13,outline:"none"}}><option value="">— Select Country —</option>{countries.map(c=><option key={c} value={c}>{c}</option>)}</select>
      ...
      {!selectedCountry?(
        <div style={{background:"#FFF",borderRadius:10,padding:40,textAlign:"center",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><div style={{fontSize:36,marginBottom:12}}>🌍</div><div style={{fontSize:14,fontWeight:600,color:"#333",marginBottom:6}}>{countries.length} countries</div><div style={{fontSize:11,color:"#999",marginBottom:16}}>Select above</div><div style={{display:"flex",flexWrap:"wrap",gap:6,justifyContent:"center",maxWidth:400,margin:"0 auto"}}>{countries.map(c=><button key={c} onClick={()=>setSelectedCountry(c)} style={{padding:"5px 12px",borderRadius:16,border:"1px solid #2CADA6",background:"rgba(44,173,166,0.08)",color:"#2CADA6",fontSize:11,fontWeight:600,cursor:"pointer"}}>{c}</button>)}</div></div>
      ):(
        <div style={{background:"#FFF",borderRadius:8,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><div style={{background:"#2CADA6",padding:"10px 14px"}}><span style={{fontSize:13,fontWeight:700,color:"#FFF"}}>🌍 {selectedCountry} — {filtered.length} ranges</span></div>
        <table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr>{["SL","PREFIX","PRICE","SUPPLIER","TEST NUMBER"].map((h,i)=><th key={i} style={thS}>{h}</th>)}</tr></thead>
        <tbody>{filtered.map((r,i)=>{const sym=r.currency==="USD"?"$":"€";const testNum=getTest(r);return(<tr key={r.id} style={{borderBottom:"1px solid #F5F5F5",background:i%2===0?"#FFF":"#FAFAFA"}}>
          <td style={{padding:"8px 10px",fontSize:11,color:"#999",fontWeight:600}}>{i+1}</td>
          ...
          <td style={{padding:"8px 10px"}}><span style={{fontSize:12,fontFamily:"monospace",fontWeight:700,marginRight:8}}>{testNum}</span><button onClick={()=>{navigator.clipboard?.writeText(testNum);setMsg("Copied: "+testNum);setTimeout(()=>setMsg(null),2000);}} style={{padding:"2px 6px",borderRadius:4,border:"1px solid #2CADA6",background:"rgba(44,173,166,0.1)",color:"#2CADA6",fontSize:9,fontWeight:700,cursor:"pointer"}}>Copy</button></td>
        </tr>);})}</tbody></table></div>
      )}
    </div>
  </div>);
...
function AddTestNumberPage({token}){
  ...
  return(<div style={{minHeight:"100vh",background:"#F2F2F2",fontFamily:"Arial,Helvetica,sans-serif"}}>
    <div style={{background:"#FFF",borderBottom:"1px solid #E0E0E0",padding:"12px 16px"}}><div style={{fontSize:18,fontWeight:700}}>➕ Add Test Number</div><div style={{fontSize:11,color:"#999",marginTop:2}}>Single or bulk</div></div>
    <div style={{padding:"12px 16px"}}>
      ...
      <div style={{display:"flex",gap:4,marginBottom:12}}>{[["single","Single"],["bulk","Bulk"]].map(([m,l])=>(<button key={m} onClick={()=>setAddMode(m)} style={{padding:"9px 18px",borderRadius:20,border:"none",fontSize:12,background:addMode===m?"#2CADA6":"#FFF",color:addMode===m?"#FFF":"#555",fontWeight:addMode===m?700:400,cursor:"pointer",boxShadow:"0 1px 4px rgba(0,0,0,0.08)"}}>{l}</button>))}</div>
      {addMode==="single"&&<div style={{background:"#FFF",borderRadius:10,padding:16,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
        ...
        <button onClick={async()=>{if(!newTest.number||!newTest.supplier_id||!newTest.country||!newTest.rate){alert("All required");return;}setSaving(true);const num=newTest.number.startsWith("+")?newTest.number:"+"+newTest.number;const d=await apiFetch("/dids",token,{method:"POST",body:JSON.stringify({number:num,prefix:newTest.prefix,country_name:newTest.country,tariff:parseFloat(newTest.rate),selling_price:parseFloat(newTest.rate),currency:newTest.currency,trunk_id:parseInt(newTest.supplier_id),payment_terms:"Weekly",ivr_context:"custom/6g-premium-telecom"})});setSaving(false);setMsg(d.success||d.message?{ok:true,text:"Added: "+num}:{ok:false,text:"Failed"});if(d.success||d.message)setNewTest({number:"",country:"",prefix:"",rate:"",currency:"EUR",supplier_id:""});setTimeout(()=>setMsg(null),3000);}} disabled={saving} style={{width:"100%",padding:"12px",borderRadius:8,border:"none",background:"#2CADA6",color:"#FFF",fontSize:14,fontWeight:700,cursor:"pointer"}}>{saving?"Saving...":"✅ Add"}</button>
      </div>}
      {addMode==="bulk"&&<div style={{background:"#FFF",borderRadius:10,padding:16,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
        ...
        <div style={{marginBottom:12}}><div style={{fontSize:11,fontWeight:600,color:"#555",marginBottom:5}}>Numbers (number, country, prefix, rate, currency)</div><div style={{fontSize:10,color:"#999",marginBottom:6,padding:"6px 10px",background:"#F8F8F8",borderRadius:6,fontFamily:"monospace"}}>+88233770042, Satellite, 88233770, 0.42, EUR</div><textarea rows={8} style={{...inp,fontFamily:"monospace",fontSize:11,resize:"vertical"}} value={bulkText} onChange={e=>setBulkText(e.target.value)}/></div>
        <button onClick={async()=>{if(!bulkSupplier||!bulkText.trim()){alert("Required");return;}setSaving(true);const lines=bulkText.trim().split("\n").filter(l=>l.trim());let ok=0;for(const line of lines){const p=line.split(",").map(x=>x.trim());const num=p[0]?.startsWith("+")?p[0]:"+"+p[0];const d=await apiFetch("/dids",token,{method:"POST",body:JSON.stringify({number:num,country_name:p[1]||"Unknown",prefix:p[2]||(num.replace("+","").slice(0,-4)),tariff:parseFloat(p[3])||0.42,selling_price:parseFloat(p[3])||0.42,currency:p[4]||"EUR",trunk_id:parseInt(bulkSupplier),payment_terms:"Weekly",ivr_context:"custom/6g-premium-telecom"})});if(d.success||d.message)ok++;}setSaving(false);setMsg({ok:true,text:"Imported "+ok});setBulkText("");setBulkSupplier("");setTimeout(()=>setMsg(null),5000);}} disabled={saving} style={{width:"100%",padding:"12px",borderRadius:8,border:"none",background:"#2CADA6",color:"#FFF",fontSize:14,fontWeight:700,cursor:"pointer"}}>{saving?"Importing...":"📋 Import"}</button>
      </div>}
    </div>
  </div>);
}
...
function TestLiveCallPage({token}){
  ...
  const thS={fontSize:9,color:"#888",fontWeight:600,padding:"8px 10px",textAlign:"left",borderBottom:"2px solid #E8E8E8",background:"#F5F5F5",textTransform:"uppercase",whiteSpace:"nowrap"};
  return(<div style={{minHeight:"100vh",background:"#F2F2F2",fontFamily:"Arial,Helvetica,sans-serif"}}>
    <div style={{background:"#FFF",borderBottom:"1px solid #E0E0E0",padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:18,fontWeight:700}}>📞 Live Test Call</div><div style={{fontSize:11,color:"#999",marginTop:2}}>{calls.length>0?<span style={{color:"#10B981",fontWeight:700}}>● {calls.length} active</span>:"● No calls"} · Auto 5s</div></div><button onClick={load} style={{padding:"7px 14px",borderRadius:20,border:"2px solid #2CADA6",background:"#FFF",color:"#2CADA6",fontSize:11,fontWeight:700,cursor:"pointer"}}>↻</button></div>
    <div style={{padding:"12px 16px"}}>
      {calls.length===0?<div style={{background:"#FFF",borderRadius:10,padding:60,textAlign:"center",boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}><div style={{fontSize:36,marginBottom:12}}>📞</div><div style={{fontSize:14,fontWeight:600,color:"#333",marginBottom:6}}>No Active Calls</div><div style={{fontSize:12,color:"#999"}}>Make a test call to see it here</div></div>
      :<div style={{background:"#FFF",borderRadius:8,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr>{["SL","CLI","DID","COUNTRY","SUPPLIER","IVR","DURATION"].map((h,i)=><th key={i} style={thS}>{h}</th>)}</tr></thead><tbody>{calls.map((c,i)=>{const did=(c.did||c.exten||"").replace("+","");return(<tr key={i} style={{borderBottom:"1px solid #F0F0F0",background:i%2===0?"#FFF":"#F9FFFE"}}><td style={{padding:"8px 10px",fontSize:11,color:"#999",fontWeight:600}}>{i+1}</td><td style={{padding:"8px 10px",fontSize:11,fontFamily:"monospace",fontWeight:600}}>{c.src||"—"}</td><td style={{padding:"8px 10px",fontSize:11,fontFamily:"monospace",color:"#2CADA6",fontWeight:700}}>{did}</td><td style={{padding:"8px 10px",fontSize:11}}>{c.country||"—"}</td><td style={{padding:"8px 10px",fontSize:11,color:"#2CADA6",fontWeight:600}}>{c.supplier||c.trunk_name||"—"}</td><td style={{padding:"8px 10px",fontSize:10,color:"#555"}}>{(c.ivr||c.ivr_context||"—").replace("custom/","")}</td><td style={{padding:"8px 10px",fontSize:12,fontWeight:700,color:"#10B981",fontFamily:"monospace"}}>{fmt(Math.min(86400,parseInt(c.seconds||c.billsec||0)))}</td></tr>);})}</tbody></table></div>}
    </div>
  </div>);
}
...
function TestAccessListPage({token}){
  ...
  const inp={width:"100%",padding:"8px 10px",border:"1px solid #E0E0E0",borderRadius:6,fontSize:12,outline:"none",boxSizing:"border-box"};
  return(<div style={{minHeight:"100vh",background:"#F2F2F2",fontFamily:"Arial,Helvetica,sans-serif"}}>
    <div style={{background:"#FFF",borderBottom:"1px solid #E0E0E0",padding:"12px 16px"}}><div style={{fontSize:18,fontWeight:700}}>🔐 Access List</div><div style={{fontSize:11,color:"#999",marginTop:2}}>Manage caller CLIs</div></div>
    <div style={{padding:"12px 16px"}}>
      ...
      :<div style={{background:"#FFF",borderRadius:8,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr>{["CLI","NAME","COMPANY","TYPE","NOTE","ADDED",""].map((h,i)=><th key={i} style={thS}>{h}</th>)}</tr></thead><tbody>{list.map((e,i)=>(<tr key={e.id} style={{borderBottom:"1px solid #F5F5F5",background:i%2===0?"#FFF":"#FAFAFA"}}><td style={{padding:"8px 10px",fontSize:12,fontFamily:"monospace",fontWeight:600}}>{e.cli}</td><td style={{padding:"8px 10px",fontSize:11}}>{e.name||"—"}</td><td style={{padding:"8px 10px",fontSize:11,color:"#555"}}>{e.company||"—"}</td><td style={{padding:"8px 10px"}}><span style={{fontSize:10,padding:"2px 8px",borderRadius:10,fontWeight:700,background:tC(e.type)+"20",color:tC(e.type)}}>{tI(e.type)} {e.type.toUpperCase()}</span></td><td style={{padding:"8px 10px",fontSize:11,color:"#555"}}>{e.note||"—"}</td><td style={{padding:"8px 10px",fontSize:10,color:"#999"}}>{(e.created_at||"").slice(0,10)}</td><td style={{padding:"8px 10px",textAlign:"center"}}><button onClick={async()=>{if(!window.confirm("Remove?"))return;await apiFetch("/test/access-list/"+e.id,token,{method:"DELETE"});load();}} style={{background:"none",border:"1px solid #EF4444",borderRadius:4,cursor:"pointer",fontSize:10,color:"#EF4444",padding:"2px 6px"}}>Del</button></td></tr>))}</tbody></table></div>}
    </div>
  </div>);
}
